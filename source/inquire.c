...
void Print_stu_credit(ListNode* p) {
    for (int i = 0; i < p->QualityGradeNum; i++) {
        if (p->QGrade[i]->QualityGradeType == THESIS) {
            printf("论文名称:%s 出版时间:%d 论文等级:%d 期刊名称:%s ", p->QGrade[i]->Thesis->thesisName, p->QGrade[i]->Thesis->publicationTime, p->QGrade[i]->Thesis->Grade, p->QGrade[i]->Thesis->journalName);
        }
        else if (p->QGrade[i]->QualityGradeType == PROJECT) {
            printf("项目名:%s 项目编号:%d 开题时间:%d 结题时间:%d ", p->QGrade[i]->Project->projrectName, p->QGrade[i]->Project->itemNum, p->QGrade[i]->Project->approvalTime, p->QGrade[i]->Project->endTime);
        }
        else if (p->QGrade[i]->QualityGradeType == CONTEST) {
            printf("竞赛名:%s 主办单位:%s 竞赛等级:%d 获奖时间:%d ", p->QGrade[i]->Contest->contestName, p->QGrade[i]->Contest->organizer, p->QGrade[i]->Contest->conG, p->QGrade[i]->Contest->prizeTIme);
        }
...
void file_Inquire(ListNode* head, ListNode* tailer) {
    ...
    printf("查询完毕！结果已输出到'result.txt'文件(源文件目录下)\n");
    fclose(fp1);
    fclose(fp);
    system("pause");
    return;
